  ...
  <div style="height: calc(100vh - 150px); background-color:#a7b8a3">
    <v-row style="width: 100%; padding: 50px">
      <div v-if="isOrganizer" style="width: 50%">
        <v-text-field clearable label="Trip name" v-model="name" :rules="nameRules" counter="100"></v-text-field>
        <v-textarea
          clearable
          label="Trip descryption"
          v-model="descryption"
      ...
      <div style="width: 50%; padding-left: 50px">
        ...
        <v-btn
          v-if="this.participant!=null"
          id="deleteUserbtn"
          retain-focus-on-click
          color="#548561"
          x-large
          block
        >Remove {{participant.username}} from trip</v-btn>
        <b-popover
          v-if="this.participant!=null"
          target="deleteUserbtn"
          triggers="focus"
          placement="bottom"
        >
    ...
    <v-btn
      id="deleteTripbtn"
      retain-focus-on-click
      style="width: calc(100% - 70px); position: absolute; bottom: 0px; left: 35px;"
      v-if="isOrganizer"
      color="#548561"
      x-large
    >Delete trip</v-btn>
...
export default {
  data: function() {
    return {
      ...
      nameRules: [
        v => v.length >= 10 || "Name must be minimum 10 characters long",
        v => v.length <= 100 || "Name must be maximum 100 characters long"
      ],
      descryptionRules: [
        v => v.length <= 5000 || "Name must be maximum 5000 characters long"
      ]
    };
  },
  computed: {
  ...
  methods: {
    ...
    async removeParticipant() {
      axios
        ...
        .then(response => {
          this.$toasted.show(
            "Participant " + response.data.username + " removed",
            {
              theme: "outline",
              position: "top-right",
              duration: 1000
