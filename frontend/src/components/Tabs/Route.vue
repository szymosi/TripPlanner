    ...
    <div style="padding:30px; width:40%; height:100%; float: left;">
        ...
        <b-list-group>
            <list v-for="controlPoint in controlPoints" v-bind:key="controlPoint">
                <b-list-group-item id="ListElement" button @click="selectPoint(controlPoint)">{{controlPoint.order}} {{controlPoint.name}}</b-list-group-item>
            </list>
        </b-list-group>
        <v-pagination
          id="pagination"
...
export default {
  data() {
    return {
      ...
      name: "",
      nameRules: [v => v.length <=50 || "Name must be maximum 50 characters long"],
    ...
    getAllControlPoints: function() {
      ...
      .then(response => {
        ...
        response.data.forEach(element => {
          this.markers.push({position:({lat: element.latitude, lng: element.longitude })});
        });
      })
      .catch(error=>{
        this.showError(error)
