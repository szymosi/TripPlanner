    ...
    <div style="height: calc(100vh - 150px); background-color:#a7b8a3">
        <div style="padding:15px; width:50%; float: left;">
            ...
            <div v-if="update && isOrganizer">
                ...
                <v-text-field v-model=expense.cost label="Cost" solo full-width dense>Cost</v-text-field>
            ...
            <template v-slot:append="{item}">
                ...
                <i @click=deleteExpense(item) class="fas fa-trash" style="padding: 5px; color:#ffffff"></i>
...
export default {
  ...
  methods: {
    ...
    async getBudget(){
        axios.get(this.$url +'/'+ this.$store.getters.trip.id + '/Budget',{
        headers:{
    ...
    updateBudget(){
        ...
        axios.post(this.$url +'/'+ this.$store.getters.trip.id + '/Budget/BudgetUpdate?'+
        'founds='+this.founds
    )
    .then(response => {
        this.founds=response.data.founds;
    ...
    async addExpense(){
        axios.defaults.headers.common['Authorization']='Bearer:'+this.$store.getters.token,
        axios.put(this.$url +'/'+ this.$store.getters.trip.id + '/Budget/ExpenseAdd?'+
        'parentExpense='+this.expense.id
        ,{
            cost: this.cost,
            name: this.name,
        }
    ...
    async deleteExpense(expense){
        axios.delete(this.$url +'/'+ this.$store.getters.trip.id + '/Budget/ExpenseDelete',{
        headers:{
          Authorization: 'Bearer:'+this.$store.getters.token
        },
        params:{
    ...
    async updateExpense(){
        axios.defaults.headers.common['Authorization']='Bearer:'+this.$store.getters.token,
        axios.post(this.$url +'/'+ this.$store.getters.trip.id + '/Budget/ExpenseUpdate?'
